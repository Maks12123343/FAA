...
def _call_claude(system: str, messages: list, timeout: int = 180) -> tuple:
    """Windows: GigaCoder primary, Pioneer fallback. Linux: Pioneer primary, GigaCoder fallback."""
    if platform.system() == "Windows":
        try:
            return api_client.call_gigacoder(system, messages, timeout=timeout)
        except Exception as e:
            print(f"[writer] GigaCoder failed ({e}), falling back to Pioneer", flush=True)
