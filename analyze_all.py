"""
Аналіз ВСІХ кліпів Kung Fu Panda що не мають analysis.json.
Підтримує обидва формати: clip.analysis.json і clip.mp4.analysis.json
"""
import base64
import json
import os
...
def main():
    ...
    # List all mp4 clips
    print("Listing all mp4 clips via rclone...", flush=True)
    all_mp4 = sorted(_rclone_list(RCLONE_CLIPS, "*.mp4"))
    print(f"Total clips: {len(all_mp4)}", flush=True)

    # List all existing analysis files (both formats)
    print("Listing existing analysis files...", flush=True)
    all_analysis = _rclone_list(RCLONE_CLIPS, "*.analysis.json")

    done_set = set()
    for fn in all_analysis:
        # Handle both formats:
        # clip.analysis.json → clip
        # clip.mp4.analysis.json → clip
        base = fn
        if base.endswith(".mp4.analysis.json"):
            base = base[:-len(".mp4.analysis.json")]
        elif base.endswith(".analysis.json"):
            base = base[:-len(".analysis.json")]
        done_set.add(base)

    print(f"Already analyzed (GDrive): {len(done_set)}", flush=True)
    ...
    all_done = done_set | local_done
    missing = [
        os.path.join(CLIPS_DIR, f) for f in all_mp4
        if f.replace(".mp4", "") not in all_done
    ]
    print(f"Need analysis: {len(missing)}", flush=True)

    if not missing:
        print("All clips already analyzed!", flush=True)
